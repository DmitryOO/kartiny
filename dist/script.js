...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Object(_modules_shoMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '#styles .row');
...
/* harmony default export */ __webpack_exports__["default"] = (buger);
...
const drop = () => {
  ...
  //dragover - обьект протаскивают над dropArea(каждые 100ms) 
  //dragstart*
  //drop - обьект отправлен в dropArea(отпустили кнопку мыши)
  ...
  function highLight(item) {
    item.closest('.file_upload').style.border = '5px solid cyan';
...
const filter = () => {
  const menu = document.querySelector('.portfolio-menu'),
        ...
        arrPics = [[btnAll, markAll], [btnLovers, markLovers], [btnChef, markChef], [btnGirl, markGirl], [btnGranddad, ''], [btnGrandmother, ''], [btnGuy, markGuy]];
...
const scrolling = upSelector => {
  ...
  window.addEventListener('scroll', () => {
    if (document.documentElement.scrollTop > 1650) {
      ...
      upElem.classList.remove('fadeIn', 'animated');
    }
  ...
  //     let scrollTop = (body.scrollTop || element.scrollTop);
  //     if (this.hash != '') {
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn = document.querySelector(trigger); // cards.forEach(item => {
  ...
  function createCards(response) {
    response.forEach(({
      ...
      card.classList.add('animated', 'zoomInUp', 'col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1');
      card.innerHTML = `  
